...
const flagAssetUrls = import.meta.glob("../assets/*/index.svg", {
  eager: true,
  import: "default",
  query: "?url",
});
...
function flagUrl(settlement) {
  const url = flagAssetUrls[`../assets/${settlement.slug}/index.svg`];
  if (!url) {
    throw new Error(`Example asset is missing: assets/${settlement.slug}/index.svg`);
  }
  return url;
}

function setFlagImage(image, frame, settlement) {
  try {
    image.src = flagUrl(settlement);
  } catch (error) {
    console.error(error);
...
function makeFlagCell(settlement) {
  ...
  frame.addEventListener("click", () => {
    openFlagModal(settlement);
  });
  setFlagImage(image, frame, settlement);
